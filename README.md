...
- **Учёт ловушек данных:** 444 узла, обрезанных 4-м коленом, никогда не считаются конечными получателями; у seed не используется доля пропуска, потому что их входящие занижены выгрузкой.
...
```
gid 100000004015047100: CONSOLIDATOR (точка консолидации), уверенность 0.92, приоритет 1.00, кластер 5
...
python -m pytest -q tests                             # 6 тестов
```
...
Архивы собраны до появления ассистента и новых вкладок — см. [Ограничения](#ограничения).
...
Затем `python -m pytest -q tests` → `6 passed`. Повторный прогон даёт те же CSV: алгоритмы детерминированы.
...
| **Периферия** (`peripheral`) | ни одно правило не сработало | подтипы в evidence: обрезан 4-м коленом, seed без переводов, ниже порогов | 1 965 |
...
| Особенность | Решение |
|---|---|
| 444 узла с 4-го колена без исходящих — артефакт обхода | никогда не конечный получатель (проверяется тестом); периферия с пометкой «обход остановлен, сток не подтверждён»; приоритет ×0,3 |
...
| Опц.: учёт обрыва на 4-м колене | правило конечного получателя, штраф приоритета | тест `test_truncated_not_terminal` |
...
- Оценка полноты данных как отдельная функция («каких данных не хватает, какой запрос сделать»).
...
- Роли на экране и в CSV записаны кодами из словаря ТЗ (`consolidator` и т. д.); русские названия — в легенде и объяснениях.
- Офлайн-архивы в ветке `offline-bundles` собраны до появления ассистента и новых вкладок. Для актуальной версии их нужно пересобрать (`bash tools/build_portable.sh`).
