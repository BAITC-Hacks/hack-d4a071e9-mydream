...
```
moneygraph/      пайплайн: load → features → clusters → roles → priority → export; explain; cli
...
1. `python -m moneygraph run` — живой прогон, 4 секунды.
2. В UI: gid из топа — консолидатор с 3 seed-плательщиками, схема окрестности, крупнейшие плательщики.
...
5. Жюри называет любой gid → `explain <gid>`.
